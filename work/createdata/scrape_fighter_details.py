...
CSV_PATH = BASE_PATH/'fighter_details_test.csv'

def get_fighter_group_urls(event) -> List[str]:
	# alphas = [chr(i) for i in range(ord('a'),ord('a')+26)]
	fighter_group_urls = [event]
...
def create_fighter_data_csv(event) -> None:
	fighter_group_urls = get_fighter_group_urls(event)
